export generate_grid, generate_ABC,assembly_metakernel,InitializePDESolver,startup_CPU,full_algorithm,update_component!
...
function assembly_metakernel(::Type{CPUBackend},
    ...
    lp = LinearProblem{isinplace}(B,ones(eltype(B),size(B,1)))
...
function InitializePDESolver(
    ::Type{Backend},
    Grid::MetaMesh,
    Kernels,
    OperatorA::OperatorOrMatrix = I,
) where {Backend<:AbstractBackend}
...
function calculate_N(PDE::SchrodingerPDENonPolynomic, currentMove, nextMove, index::Int)
    ...
    elseif similarities == size(similarities, 1)
        ...
        output[non_similar_indexes] .=
            (eval_field(PDE, next_non_similar) .- eval_field(PDE, current_non_similar)) ./
            (diff_non_similar)
...
function update_component!(
    ...
    for l = 1:max_iterations
        ...
        b .=
            b0 .-
            τ .* opA * (calculate_N(PDE, currentMove, nextMove, index) .* (zl .+ prev_zl))
...
function full_algorithm(Solver::PDESolver, PDE::SchrodingerPDE, Grid::SpaceTimeGrid, Ψⁿ)
    ...
    @showprogress for time_step in 1:time_steps
